...
import 'machine-qr-scan.dart';

class MachineQrScanStateController {
  MachineQrScanStateController({required this.state}) {}
  ...
  // Counting pointers (number of login-info.dart fingers on screen)
  int _pointers = 0;
  ...
  MachineQrScanState state;

  Future<void> startQRScan() async {
    await startBackCamera();
    var scanCompleted = false;
    BarcodeScanningService.fromCamera(controller!, (barcode) {
      if (scanCompleted) {
        return false;
      }
      AddMachineResponse response = AddMachineResponse(
          state: AddMachineState.SUCCESS,
          deviceCode: barcode
      );
      if(state.mounted){
        scanCompleted = true;
  ...
  Future<void> startBackCamera() async {
    for (var camDescription in DevicesInfo.info.cameras) {
      if (camDescription.lensDirection == CameraLensDirection.back) {
        await onNewCameraSelected(camDescription);
        break;
      }
    }
  ...
  Future<XFile?> takePicture() async {
    ...
    if (cameraController == null || !cameraController.value.isInitialized) {
      showInSnackBar('Xin lỗi làm ơn mở camera.');
      return null;
    }
